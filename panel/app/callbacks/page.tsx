...
> = {
  high:   { label: "Hoch",    bg: "#fee2e2", text: "#b91c1c", border: "#fca5a5", dot: "#dc2626" },
  medium: { label: "Mittel",  bg: "#fef3c7", text: "#b45309", border: "#fcd34d", dot: "#d97706" },
  low:    { label: "Niedrig", bg: "#f8fafc", text: "#64748b", border: "#e2e8f0", dot: "#94a3b8" },
};
...
export default function CallbacksPage() {
  ...
  useEffect(() => {
    fetch("/api/callbacks").then((r) => r.json()).then((d) => setItems(d.callbacks));
  }, []);
  ...
  return (
    <div className="space-y-4">
      <p className="flex items-center gap-2 text-sm text-subtle">
        ...
        <span><strong className="text-text">{items.length}</strong> geplante Rückrufe</span>
      </p>

      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {items.map((cb) => {
          const p = PRIO[cb.priority];
          return (
            <div key={cb.id} className="card animate-slideUp p-4 hover:shadow-cardHover transition-shadow">
              {/* Header */}
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <button className="rounded-lg bg-brand px-3 py-1.5 text-xs font-medium text-white hover:bg-brandDark transition-colors">
                  Anrufen
                </button>
              </div>
            </div>
